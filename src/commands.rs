...
use crate::project::manager::{ProjectManager, ProjectManagerError};
...
use crate::ai::generator::{SceneGenerator, GenerationParams, GeneratedScene};
...
#[command]
pub async fn get_active_project(state: State<'_, AppState>) -> Result<CommandResponse<Project>, String> {
    let project_manager = state.project_manager.lock().unwrap();
...
pub async fn create_project(
    ...
    match project_manager.create_project(&name, author.as_deref()) {
        ...
        Err(e) => Ok(CommandResponse::error(&format!("Failed to create project: {}", e))),
    }
}
...
pub async fn create_template_project(
    ...
    match project_manager.create_from_template(&name, author.as_deref()) {
        Ok(id) => Ok(CommandResponse::success(id)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to create template project: {}", e))),
    }
}
...
pub async fn load_project(
    ...
    match project_manager.load_project(&id) {
        Ok(project) => Ok(CommandResponse::success(project)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to load project: {}", e))),
    }
}
...
pub async fn save_project(state: State<'_, AppState>) -> Result<CommandResponse<bool>, String> {
    ...
    match project_manager.save_active_project() {
        ...
        Err(e) => Ok(CommandResponse::error(&format!("Failed to save project: {}", e))),
    }
}
...
pub async fn import_project(
    ...
    match project_manager.import_project(PathBuf::from(path)) {
        Ok(id) => Ok(CommandResponse::success(id)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to import project: {}", e))),
    }
}
...
pub async fn export_project(
    ...
    match project_manager.export_active_project(PathBuf::from(path)) {
        Ok(_) => Ok(CommandResponse::success(true)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to export project: {}", e))),
    }
}
...
pub async fn close_project(state: State<'_, AppState>) -> Result<CommandResponse<bool>, String> {
    ...
    match project_manager.close_active_project() {
        Ok(_) => Ok(CommandResponse::success(true)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to close project: {}", e))),
    }
}
...
pub async fn create_scene(
    ...
    match project_manager.create_scene(&name, description.as_deref()) {
        Ok(id) => Ok(CommandResponse::success(id)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to create scene: {}", e))),
    }
}
...
pub async fn activate_scene(
    ...
    match project_manager.activate_scene(&id) {
        Ok(_) => Ok(CommandResponse::success(true)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to activate scene: {}", e))),
    }
}
...
pub async fn assign_scene_to_grid(
    ...
    match project_manager.assign_scene_to_grid(&scene_id, position) {
        Ok(_) => Ok(CommandResponse::success(true)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to assign scene to grid: {}", e))),
    }
}
...
#[command]
pub async fn list_midi_devices(state: State<'_, AppState>) -> Result<CommandResponse<Vec<MidiDevice>>, String> {
    let project_manager = state.project_manager.lock().unwrap();
    ...
    match ProjectManager::get_midi_devices() {
        ...
        Err(e) => Ok(CommandResponse::error(&format!("Failed to get MIDI devices: {}", e))),
    }
}
...
pub async fn connect_controller(
    ...
    match project_manager.connect_controller(&device_id) {
        Ok(_) => Ok(CommandResponse::success(true)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to connect controller: {}", e))),
    }
}

#[command]
pub async fn disconnect_controller(state: State<'_, AppState>) -> Result<CommandResponse<bool>, String> {
    let mut project_manager = state.project_manager.lock().unwrap();

    match project_manager.disconnect_controller() {
        Ok(_) => Ok(CommandResponse::success(true)),
        Err(e) => Ok(CommandResponse::error(&format!("Failed to disconnect controller: {}", e))),
    }
}
...
pub async fn generate_scene(
    ...
    match scene_generator.generate(params) {
        ...
        Err(e) => Ok(CommandResponse::error(&format!("Failed to generate scene: {}", e))),
    }
}
    ...
    match project_manager.get_active_project() {
        Ok(mut project) => {
            ...
            match project_manager.save_active_project() {
                ...
                Err(e) => Ok(CommandResponse::error(&format!("Failed to save scene: {}", e))),
            }
        },
        Err(e) => Ok(CommandResponse::error(&format!("No active project: {}", e))),
    }
