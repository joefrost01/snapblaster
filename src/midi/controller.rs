...
impl GridController for LaunchpadMk2 {
    ...
    fn connect(&mut self) -> Result<(), String> {
        ...
        let in_port = in_ports.iter()
            .find(|p| {
                midi_in.port_name(p).map(|name| name == self.device.name).unwrap_or(false)
            })
        ...
        let input_conn = midi_in.connect(in_port, "launchpad-input", callback, ()).map_err(|e| e.to_string())?;
        ...
        let out_port = out_ports.iter()
            .find(|p| {
                midi_out.port_name(p).map(|name| name == self.device.name).unwrap_or(false)
            })
            .ok_or_else(|| format!("Could not find MIDI output device: {}", self.device.name))?;

        let output_conn = midi_out.connect(out_port, "launchpad-output").map_err(|e| e.to_string())?;
        ...
        if let Some(ref mut conn) = self.output_connection {
            conn.send(&[0xF0, 0x00, 0x20, 0x29, 0x02, 0x18, 0x22, 0x00, 0xF7]).map_err(|e| e.to_string())?;
    ...
    fn disconnect(&mut self) -> Result<(), String> {
        // Reset the Launchpad
        if let Some(ref mut conn) = self.output_connection {
            conn.send(&[0xF0, 0x00, 0x20, 0x29, 0x02, 0x18, 0x0E, 0x00, 0xF7]).map_err(|e| e.to_string())?;
        }
    ...
    fn set_pad_color(&mut self, grid_id: u8, color: Color) -> Result<(), String> {
        ...
        if let Some(ref mut conn) = self.output_connection {
            ...
            conn.send(&[0x90, launchpad_id, color_value]).map_err(|e| e.to_string())?;
        }
    ...
    fn set_button_color(&mut self, button_id: u8, color: Color) -> Result<(), String> {
        ...
        if let Some(ref mut conn) = self.output_connection {
            ...
            conn.send(&[0xB0, button_id, color_value]).map_err(|e| e.to_string())?;
        }
...
impl ControllerFactory {
    pub fn create_controller(device: MidiDevice) -> Result<Box<dyn GridController>, String> {
        if !device.is_controller {
            return Err(format!("Device '{}' is not a supported controller", device.name));
        }
